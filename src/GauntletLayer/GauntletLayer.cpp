...
BetterGauntletLayer::~BetterGauntletLayer() {
    ...
    if (m_levels) { m_levels->release(); m_levels = nullptr; }
}
...
bool BetterGauntletLayer::init(GauntletType type) {
    ...
    log::debug("BetterGauntletLayer opened for Gauntlet #{}", static_cast<int>(m_gauntletType));
    ...
    auto shadowText = CCLabelBMFont::create(titleStr.c_str(), "GR_OxygeneFontOutline.fnt"_spr);
    ...
    auto titleText = CCLabelBMFont::create(titleStr.c_str(), "GR_OxygeneFont.fnt"_spr);
    ...
    loadCircle->runAction(CCRepeatForever::create(CCRotateBy::create(1.0f, 360.0f)));
...
void BetterGauntletLayer::editGauntlets() {
    ...
    if (hover) {
        ...
        for (int i = 0; i < levelCount; i++) {
            CCNode* lvl = m_levelsMenu->getChildByIDRecursive(fmt::format("gauntlet-{}", i + 1));
            if (!lvl) continue;
